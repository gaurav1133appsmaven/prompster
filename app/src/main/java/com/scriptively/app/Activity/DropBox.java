...
public class DropBox extends DropboxActivity {
    public static final String ACCESS_TOKEN = "4Fx-B6qXQAQAAAAAAAAAAW1WwlBmoT6VstPouDqBXemUSHMgKujcCc14MvN1ESXk";
...
//        }
        ...
        try {
            ...
            new ConnectToDB(this, ACCESS_TOKEN, new ConnectToDB.Callback() {
                ...
                public void onLoginComplete(FullAccount result, DbxClientV2 client) {
                    ...
                    mClient = client;
                    Log.e(TAG, "Successdata." + client.account().getClass().getName().toString());
    ...
    public void getDropboxData(ListFolderResult result) {
        ...
        for (int i = 0; i < result.getEntries().size(); i++) {
            if (result.getEntries().get(i).getName().contains(".pdf") || result.getEntries().get(i).getName().contains(".doc") || result.getEntries().get(i).getName().contains(".docx") || result.getEntries().get(i).getName().contains(".txt")) {
                data.add(result.getEntries().get(i));

            }

        }
        mFilesAdapter.setFiles(data);
